...
public class AOC2016_02_part2 {
    public static void main(String args[]) throws Exception {
        ...
        while (scanner.hasNextLine()) {
            char[] moves = scanner.nextLine().trim().toCharArray();
            for (char move : moves) {
                ...
                nextRow = Math.min(nextRow, 4);
                nextCol = Math.max(nextCol, 0);
                nextCol = Math.min(nextCol, 4);

                char key = board[nextRow][nextCol];
                if (' ' != key) {
                    row = nextRow;
                    col = nextCol;
                }
